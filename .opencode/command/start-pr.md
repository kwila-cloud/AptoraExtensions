...
1. Gain context by reading any important files available at the project root (ARCHITECTURE.md, CONTRIBUTING.md, INFRASTRUCTURE.md, etc)
1. Branch creation rules (agents MUST NOT ask the user about branch behavior):
...
1. Research the codebase to gather information about the change.
1. Ask the user clarifying questions.
   - Clearly number the questions.
   - Clearly letter the options for each question.
1. Update the Task List section with any new updates based on your research and the user's answers.
1. Explain the current Task List section to the user.
1. When the Task List section is approved by the user, instruct the user to run `/do-pr` to begin implementing the changes. Do not use the word "proceed" as the final prompt — always reference `/do-pr`.
